...
"""
v2 wrapper around rank_corrections.

Same scoring logic as v1: for each flagged correction, add columns for
cluster size, mapped peers, current/majority votes, majority share, and
confidence gap so triage can sort by signal strength.

Reads:
    data/outputs_v2/full_clusters.csv
    data/outputs_v2/transformer_corrections.csv
    data/outputs/known_mapping.csv (shared with v1)

Writes:
    data/outputs_v2/corrections_ranked.csv
"""
...
OUT = "data/outputs_v2/corrections_ranked.csv"
...
def main():
    parser = argparse.ArgumentParser(
        description="Rank v2 flagged transformer corrections by confidence."
    )
    parser.add_argument(
        "diff_csv",
        nargs="?",
    ...
    candidates["CURRENT_TRANSFORMER"] = normalize_id(
        candidates["CURRENT_TRANSFORMER"]
    )
    ...
    candidates["MAJORITY_VOTES"] = (
        candidates["MAJORITY_VOTES"].fillna(0).astype(int)
    )
    ...
    candidates["MAPPED_PEERS"] = (
        candidates["MAPPED_PEERS"].fillna(0).astype(int)
    )
