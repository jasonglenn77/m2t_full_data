...
"""
v2 wrapper around find_mapping_errors.

Same logic as v1: for each cluster, find the majority transformer among
mapped peers and flag any badge whose GIS-assigned transformer differs.
Reads v2 clusters and the shared known_mapping, writes v2 corrections.

Reads:
    data/outputs_v2/full_clusters.csv
    data/outputs/known_mapping.csv         (shared with v1)
    data/state/corrections_ignored_v2.csv  (optional v2-specific ignore list)

Writes:
    data/outputs_v2/transformer_corrections.csv
    data/outputs_v2/corrections_suppressed.csv (if any rows suppressed)
"""
...
IGNORE_FILE = "data/state/corrections_ignored_v2.csv"
SUPPRESSED_FILE = "data/outputs_v2/corrections_suppressed.csv"
OUTPUT_FILE = "data/outputs_v2/transformer_corrections.csv"
...
if os.path.exists(IGNORE_FILE):
    ...
    if not ignore.empty and required.issubset(ignore.columns):
        ...
        rec_norm["RECOMMENDED_TRANSFORMER_NORM"] = _norm(
            rec_norm["RECOMMENDED_TRANSFORMER"]
        )
...
print("\nPotential transformer mapping errors (v2):")
print(rec_df)
