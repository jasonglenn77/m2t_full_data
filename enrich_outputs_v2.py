# enrich_outputs_v2.py
"""
v2 wrapper around enrich_outputs.

Reads v2 outputs and the shared GIS / signature-store data, writes
enriched outputs and supplementary reports into data/outputs_v2/. Same
classification logic as v1; only paths differ.
"""
...
KNOWN_MAPPING = "data/outputs/known_mapping.csv"  # shared with v1
...
def build_tx_id_lookup(sp, tx):
    """Bridge SPs to transformer metadata. Prefers OBJECTID join when
    available (100% coverage on current dataflow); falls back to TAG-based
    join for legacy files."""
    if "OBJECTID" in tx.columns:
        bridge = (
...
def write_missing_from_gis(sp, tx_lookup):
    ...
    if missing.empty:
        print("    No badges missing from GIS after recency filter.")
        pd.DataFrame(
            columns=[
                "BADGE",
                "CLUSTER",
        ...
        ).to_csv(os.path.join(OUT_DIR, "badges_missing_from_gis.csv"), index=False)
        return
...
def main():
    ...
    print("Enriching v2 correction outputs:")
    ...
    print("Producing v2 supplementary reports:")
