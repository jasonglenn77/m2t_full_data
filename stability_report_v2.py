# stability_report_v2.py
"""
v2 wrapper around stability_report.

Reads the v2 history ledger (data/state/history_v2/) and produces the
same three stability-enriched outputs into data/outputs_v2/. Same logic
as v1; only the input/output paths differ.

Outputs:
    data/outputs_v2/corrections_with_stability.csv
    data/outputs_v2/corrections_high_confidence.csv
    data/outputs_v2/badge_stability_summary.csv
"""
...
CLUSTERS = "data/outputs_v2/full_clusters.csv"
CORRECTIONS = "data/outputs_v2/transformer_corrections.csv"
RANKED = "data/outputs_v2/corrections_ranked.csv"
...
OUT_STABILITY = "data/outputs_v2/corrections_with_stability.csv"
OUT_HIGH_CONF = "data/outputs_v2/corrections_high_confidence.csv"
OUT_BADGE_STABILITY = "data/outputs_v2/badge_stability_summary.csv"
...
def main():
    if not os.path.exists(RUNS_FILE):
        raise SystemExit(
            "No v2 history ledger found at data/state/history_v2/. "
            "Run `python record_run_v2.py` at least once first."
        )
    ...
    n_runs = len(runs)
    print(f"v2 history: {n_runs} run(s) recorded.")
    if n_runs < 5:
        print(
            "  NOTE: stability scores are not very meaningful below ~5 runs. "
            "Output is still written but treat scores as preliminary."
        )
    ...
    if os.path.exists(RANKED):
        ...
        raise SystemExit("No corrections file found in data/outputs_v2/.")
